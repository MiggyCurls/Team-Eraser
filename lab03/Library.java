...
public class Library {
  ...
  public boolean checkout(long isbn, String holder, int month, int day, int year) {
	GregorianCalendar cousin = new GregorianCalendar(year, month, day);
    for(int i = 0; i < library.size(); i++) {
    	if(library.get(i).getIsbn() != 0) {
    		if(library.get(i).getHolder() == null) {
    			if(library.get(i).getIsbn() == isbn) {
    				library.get(i).checkout(holder, cousin);
        			return true;
    			}
    		}
    	}
  ...
  public ArrayList<LibraryBook> lookup(String holder) {
	  ArrayList<LibraryBook> sister = new ArrayList<LibraryBook>();
	  for(int i = 0; i < library.size(); i++) {
		  if(library.get(i).getHolder() != null) {
			  if(library.get(i).getHolder().equals(holder)) {
					sister.add(library.get(i));
				}
		  }
	  }
	  return sister;
  ...
  public boolean checkin(long isbn) {
	  for(int i = 0; i < library.size(); i++) {
		  if(library.get(i).getIsbn() == isbn) {
			if(library.get(i).mom != null) {
				library.get(i).bookHolder = null;
				library.get(i).mom = null;
				return true;
			}
		  }
	  }
