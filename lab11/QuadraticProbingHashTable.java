...
public class QuadraticProbingHashTable
{
    ...
    public void insert( int x )
    {
    	...
    	double loadFactor = (currentSize / (double)HashTable.length);
    	if(loadFactor >= 0.75) {
    		rehash();
    	}
    	int hashValue = hash(x, HashTable.length);
    	if(HashTable[hashValue] == null || !HashTable[hashValue].isActive) {
    		HashTable[hashValue] = new HashEntry(x);
    		this.currentSize++;
    		inserted = true;
    	}else if(HashTable[hashValue].element != x){
    		int i = 0;
    		while(i <= 5) {
    			if(HashTable[newHashValue(i,hashValue)] == null || !HashTable[hashValue].isActive) {
    				HashTable[newHashValue(i,hashValue)] = new HashEntry(x);
    				this.currentSize++;
    				inserted = true;
    				break;
    			}
    	...
    	loadFactor = (currentSize / (double)HashTable.length);
    	if(loadFactor >= 0.75) {
    		rehash();
    	}
    }
    private int newHashValue(int counter, int oldHashValue) {
    	return ((oldHashValue + (int)Math.pow(counter, 2)) % HashTable.length);
    }
    ...
    public void rehash( )
    {
    	HashEntry[] newHashTable = new HashEntry[2 * this.HashTable.length];
    	HashEntry[] temp = this.HashTable;
    ...
    public void remove( int x )
    {
    	...
    	while(j < HashTable.length) {
    		if(HashTable[j] != null && HashTable[j].element == x) {
    			HashTable[j].isActive = false;
    			this.currentSize--;
    			break;
    		}
    		j++;
    ...
    public int find( int x )
    {
    	int j = 0;
    	while(j < HashTable.length) {
    		int index = newHashValue(j, hash(x,HashTable.length));
    		if(HashTable[index] != null && (HashTable[index].element == x && HashTable[index].isActive)) {
    			return index;
    		}
    		j++;
    	}
