...
public class Library<Type> {
	  ...
	  public ArrayList<LibraryBook<Type>> getOrderedByAuthor() {
	    ArrayList<LibraryBook<Type>> dad = this.library;
	    AuthorComparator comparatorTwo = new AuthorComparator();
	    sort(dad, comparatorTwo);
	    return dad;	  
	  }
	  ...
	  public ArrayList<LibraryBook<Type>> getOverdueList(int month, int day,
	      int year) {
		  ArrayList<LibraryBook<Type>> bob = new ArrayList<LibraryBook<Type>>();
		  GregorianCalendar dummy = new GregorianCalendar(year, month, day);
		    for(int i = 0; i < this.library.size(); i++) {
		    	if(this.library.get(i).getDueDate() != null) {
		    		if(this.library.get(i).getDueDate().compareTo(dummy) > 0) {
			    		bob.add(this.library.get(i));
			    		System.out.println("this ran");
			    	}
		    	}
		    }
		    DueDateComparator poop = new DueDateComparator();
		    sort(bob,poop);
		    return bob;
	  }
	...
	Comparator<LibraryBook<Type>> {
	  ...
	  protected class DueDateComparator implements Comparator<LibraryBook<Type>> {
		  public int compare(LibraryBook<Type> lhs, LibraryBook<Type> rhs) {
			  GregorianCalendar left = lhs.getDueDate();
			  GregorianCalendar right = rhs.getDueDate();
